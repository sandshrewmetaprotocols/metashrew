...
use anyhow::Result;
use rockshrew_sync::{MetashrewSync, SyncConfig, MockBitcoinNode, MockStorage, MockRuntime, SyncEngine, StorageAdapter};
...
async fn test_atomic_processing_fallback() -> Result<()> {
    ...
    assert!(result.is_err(), "Processing should fail when runtime is not ready");
...
async fn test_no_duplicate_state_root_initialization() -> Result<()> {
    ...
    assert_eq!(state_root_99.unwrap(), existing_state_root, "Existing state root should not be overwritten");
