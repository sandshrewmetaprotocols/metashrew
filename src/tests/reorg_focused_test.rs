...
impl SimpleIndexer {
    ...
    pub async fn get_blocktracker_view(&self, height: u32) -> Result<Vec<u8>> {
        let view_input = vec![];
        self.runtime.view("blocktracker".to_string(), &view_input, height).await
    }
    ...
    pub async fn get_block_view(&self, height: u32) -> Result<Vec<u8>> {
        ...
        self.runtime.view("getblock".to_string(), &height_input, height).await
    }
...
async fn test_reorg_scenario_blocktracker_consistency() -> Result<()> {
    ...
    for height in 0..2 {
        ...
        assert_eq!(original_bt, reorg_bt, 
                  "Height {} should be unchanged by reorg", height);
    ...
    for height in 2..=4 {
        let original_bt = indexer.get_blocktracker_view(height).await?;
        let reorg_bt = reorg_indexer.get_blocktracker_view(height).await?;
        assert_ne!(original_bt, reorg_bt, 
                  "Height {} should be changed by reorg", height);
        assert_eq!(original_bt.len(), reorg_bt.len(), 
                  "Height {} should have same length", height);
    ...
    for height in 0..4 {
        ...
        assert_eq!(&bt_next[0..bt_current.len()], &bt_current[..], 
                  "Height {} should be prefix of height {}", height, height + 1);
    }
...
async fn test_deep_reorg_scenario() -> Result<()> {
    ...
    deep_reorg_indexer.process_blocks(&deep_reorg_blocks).await?;
    ...
    for height in 1..=5 {
        ...
        assert_ne!(original_bt, reorg_bt, 
                  "Height {} should be changed by deep reorg", height);
        assert_eq!(original_bt.len(), reorg_bt.len(), 
                  "Height {} should have same length", height);
    }
    ...
    for height in 0..=5 {
        ...
        assert_eq!(bt.len(), (height + 1) as usize, 
                  "Height {} should have correct length", height);
    }
...
async fn test_blocktracker_reflects_hash_changes() -> Result<()> {
    ...
    assert_ne!(bt_a_height_1, bt_b_height_1, 
              "Blocktracker should reflect different block hashes");
    assert_eq!(bt_a_height_1.len(), bt_b_height_1.len(), 
              "Blocktracker should have same length");

    // The difference should be in the second byte (index 1)
    assert_eq!(bt_a_height_1[0], bt_b_height_1[0], "First byte should be same (genesis)");
    assert_ne!(bt_a_height_1[1], bt_b_height_1[1], "Second byte should be different");
...
async fn test_exact_blocktracker_reorg_verification() -> Result<()> {
    ...
    for height in 0..=3 {
        ...
        assert_eq!(actual_bt, expected_bt,
                  "Initial blocktracker at height {} should match computed expectation", height);
        println!("✓ Initial height {}: expected={:?}, actual={:?}", height, expected_bt, actual_bt);
    }
    ...
    assert_ne!(initial_chain[2].block_hash(), reorg_chain[2].block_hash(),
              "Reorg block 2 should have different hash");
    assert_ne!(initial_chain[3].block_hash(), reorg_chain[3].block_hash(),
    ...
    for height in 0..=3 {
        ...
        assert_eq!(actual_bt, expected_bt,
                  "Reorg blocktracker at height {} should match computed expectation", height);
        
        // Verify the specific changes
        if height < 2 {
            // Heights 0-1 should be same as original
            let original_bt = indexer.get_blocktracker_view(height).await?;
            assert_eq!(actual_bt, original_bt,
                      "Height {} should be unchanged by reorg", height);
        } else {
            // Heights 2-3 should be different from original
            let original_bt = indexer.get_blocktracker_view(height).await?;
            assert_ne!(actual_bt, original_bt,
                      "Height {} should be changed by reorg", height);
        }
        
        println!("✓ Reorg height {}: expected={:?}, actual={:?}", height, expected_bt, actual_bt);
    }
...
async fn test_state_root_reorg_verification() -> Result<()> {
    ...
    assert_eq!(initial_state_roots[0], reorg_state_roots[0],
    ...
    for height in 1..=3 {
        if initial_state_roots[height as usize].is_some() || reorg_state_roots[height as usize].is_some() {
            assert_ne!(initial_state_roots[height as usize], reorg_state_roots[height as usize],
                      "State root at height {} should change due to reorg", height);
            println!("✓ State root changed at height {}", height);
        } else {
            println!("ℹ State root tracking not enabled at height {} (both None)", height);
        }
    }
    ...
    for height in 0..=3 {
        ...
        assert_eq!(state_root, reorg_state_roots[height as usize],
                  "Historical state root query should match stored root at height {}", height);
...
async fn test_historical_view_functions_after_reorg() -> Result<()> {
    ...
    for height in 0..=3 {
        ...
        let reorg_bt = reorg_indexer.get_blocktracker_view(height).await?;
        let expected_bt = SimpleIndexer::compute_expected_blocktracker(&reorg_chain, height);
        assert_eq!(reorg_bt, expected_bt,
                  "Historical blocktracker query should return correct reorged data at height {}", height);

        // Test getblock view function
        let reorg_block = reorg_indexer.get_block_view(height).await?;
        assert!(!reorg_block.is_empty(),
               "Historical block query should return data at height {}", height);

        // Verify changes due to reorg
        if height < 2 {
            // Heights 0-1 should be unchanged
            assert_eq!(reorg_bt, initial_blocktrackers[height as usize],
                      "Blocktracker at height {} should be unchanged", height);
            assert_eq!(reorg_block, initial_blocks[height as usize],
                      "Block at height {} should be unchanged", height);
        } else {
            // Heights 2-3 should be different
            assert_ne!(reorg_bt, initial_blocktrackers[height as usize],
                      "Blocktracker at height {} should be changed by reorg", height);
            assert_ne!(reorg_block, initial_blocks[height as usize],
                      "Block at height {} should be changed by reorg", height);
        }

        println!("✓ Historical view functions at height {} working correctly after reorg", height);
    }
    ...
    for height in 0..=3 {
        ...
        assert_eq!(bt1, bt2, "Multiple blocktracker queries should be consistent at height {}", height);
        assert_eq!(bt2, bt3, "Multiple blocktracker queries should be consistent at height {}", height);

        let block1 = reorg_indexer.get_block_view(height).await?;
        let block2 = reorg_indexer.get_block_view(height).await?;
        assert_eq!(block1, block2, "Multiple block queries should be consistent at height {}", height);
    }
...
async fn test_comprehensive_reorg_all_verifications() -> Result<()> {
    ...
    for height in 0..=4 {
        ...
        let expected_bt = SimpleIndexer::compute_expected_blocktracker(&reorg_chain, height);
        assert_eq!(actual_bt, expected_bt,
                  "Blocktracker computation should be exact at height {}", height);
    }

    // 2. Verify state root changes (if state roots are being tracked)
    let reorg_state_roots = reorg_indexer.get_all_state_roots(4)?;
    assert_eq!(initial_state_roots[0], reorg_state_roots[0], "Genesis state root unchanged");
    
    let mut state_roots_tracked = false;
    for height in 1..=4 {
        if initial_state_roots[height as usize].is_some() || reorg_state_roots[height as usize].is_some() {
            assert_ne!(initial_state_roots[height as usize], reorg_state_roots[height as usize],
                      "State root should change at height {}", height);
    ...
    for height in 0..=4 {
        ...
        assert_eq!(queried_root, reorg_state_roots[height as usize],
                  "Historical state root query should work at height {}", height);
    }
    ...
    for height in 0..=4 {
        ...
        assert!(!bt.is_empty(), "Blocktracker should not be empty at height {}", height);
        assert!(!block.is_empty(), "Block should not be empty at height {}", height);
        
        // Verify consistency with computed expectation
        let expected_bt = SimpleIndexer::compute_expected_blocktracker(&reorg_chain, height);
        assert_eq!(bt, expected_bt, "View function should match computation at height {}", height);
    }
    ...
    for height in 0..4 {
        ...
        let bt_next = reorg_indexer.get_blocktracker_view(height + 1).await?;
        assert_eq!(&bt_next[0..bt_current.len()], &bt_current[..],
                  "Prefix property should be maintained between heights {} and {}", height, height + 1);
    }
