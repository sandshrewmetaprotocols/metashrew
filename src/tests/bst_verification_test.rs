...
async fn test_bst_storage_verification() -> Result<()> {
    ...
    let test_block_1 =
        super::block_builder::create_test_block(1, test_block_0.block_hash(), b"test_block_1");
    ...
    let blocktracker_h0 = runtime
        .view("blocktracker".to_string(), &view_input, 0)
        .await?;
    println!(
        "Blocktracker at height 0: {} bytes: {:?}",
        blocktracker_h0.len(),
        blocktracker_h0
    );
    assert_eq!(
        blocktracker_h0.len(),
        1,
        "Blocktracker at height 0 should be 1 byte"
    );

    // Check blocktracker at height 1 (should be 2 bytes)
    let blocktracker_h1 = runtime
        .view("blocktracker".to_string(), &view_input, 1)
        .await?;
    println!(
        "Blocktracker at height 1: {} bytes: {:?}",
        blocktracker_h1.len(),
        blocktracker_h1
    );
    assert_eq!(
        blocktracker_h1.len(),
        2,
        "Blocktracker at height 1 should be 2 bytes"
    );

    // Check that historical queries work - blocktracker at height 0 should still return 1 byte
    let blocktracker_h0_again = runtime
        .view("blocktracker".to_string(), &view_input, 0)
        .await?;
    assert_eq!(
        blocktracker_h0_again, blocktracker_h0,
        "Historical queries should be consistent"
    );

    // Check block data at height 0 using getblock view function
    let height_0_input = vec![0, 0, 0, 0]; // Height 0 as little-endian bytes
    let block_h0 = runtime
        .view("getblock".to_string(), &height_0_input, 0)
        .await?;
    ...
    let block_h1 = runtime
        .view("getblock".to_string(), &height_1_input, 1)
        .await?;
    ...
    assert_ne!(
        block_h0, block_h1,
        "Blocks at different heights should be different"
    );
