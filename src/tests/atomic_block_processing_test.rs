...
use rocksdb::{DB, Options, TransactionDB, TransactionDBOptions, MultiThreaded};
use std::sync::Arc;
...
fn test_atomic_block_processing_requirement() -> Result<()> {
    ...
    for (key, value) in &test_data {
        ...
        println!("DEBUG: Stored key-value pair: {:?} = {:?}", String::from_utf8_lossy(key), String::from_utf8_lossy(value));
    }
    ...
    let _state_root = match smt_helper.calculate_and_store_state_root(height) {
        Ok(root) => {
            println!("DEBUG: State root calculated successfully: {}", hex::encode(root));
            root
        },
        Err(e) => {
...
fn test_start_block_state_root_initialization() -> Result<()> {
    ...
    match smt_helper.calculate_and_store_state_root(start_height) {
        ...
        Err(e) => {
            ...
            match smt_helper.calculate_and_store_state_root(start_height) {
                Ok(state_root) => {
                    println!("SUCCESS: State root calculated after initialization: {}", hex::encode(state_root));
                },
                Err(e) => {
...
fn test_proposed_atomic_solution() -> Result<()> {
    ...
    for (key, value) in &test_data {
        ...
        println!("DEBUG: Added to transaction: {:?} = {:?}", String::from_utf8_lossy(key), String::from_utf8_lossy(value));
    }
    ...
    println!("DEBUG: Added state root to transaction: {}", hex::encode(state_root));
    ...
    println!("DEBUG: Added block hash to transaction: {}", hex::encode(block_hash));
