...
use rockshrew_sync::{
    MockFilesystem, MockSnapshotClient, MockSnapshotConsumer, MockSnapshotProvider,
    MockSnapshotServer, RepoConfig, SnapshotClient, SnapshotConfig, SnapshotConsumer,
    SnapshotProvider, SnapshotServer,
};
...
async fn test_snapshot_server_client() -> Result<()> {
    ...
    server
        .register_snapshot(metadata.clone(), test_data.clone())
        .await?;
    ...
    // Download metadata
    let downloaded_metadata = client
        .download_metadata("http://localhost:8080/snapshots/100/metadata")
        .await?;
    ...
    let downloaded_data = client
        .download_data("http://localhost:8080/snapshots/100/data")
        .await?;
...
async fn test_network_failures() -> Result<()> {
    ...
    server
        .register_snapshot(metadata, b"test_data".to_vec())
        .await?;
    ...
    assert!(
        failures > successes,
        "Expected more failures than successes with 80% failure rate"
    );
    println!(
        "  📊 Failures: {}, Successes: {} (as expected with 80% failure rate)",
        failures, successes
    );

    // Test with 0% failure rate
    let client_reliable = MockSnapshotClient::new(server_arc).with_failure_rate(0.0);

    let available = client_reliable
        .check_repository("http://localhost:8080")
        .await?;
    assert!(available);

    let snapshots = client_reliable
        .list_remote_snapshots("http://localhost:8080")
        .await?;
