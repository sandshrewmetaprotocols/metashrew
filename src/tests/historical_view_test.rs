...
async fn test_historical_view_queries() {
    ...
    for query_height in 0..blocks.len() {
        let result = runtime.view(
            ...
            &vec![],
            query_height as u32,
        ).await.expect(&format!("Failed to query at height {}", query_height));

        println!("Height {}: {} bytes: {:?}", query_height, result.len(), result);
        ...
        assert_eq!(result.len(), expected_length, 
            ...
            query_height, expected_length, result.len());
    }

    // Test querying at current height (should have all 5 bytes)
    let current_result = runtime.view(
        "blocktracker".to_string(),
        &vec![],
        4, // Current height
    ).await.expect("Failed to query at current height");

    println!("Current (height 4): {} bytes: {:?}", current_result.len(), current_result);
    assert_eq!(current_result.len(), 5, "Current height should have 5 bytes");

    // Test querying beyond current height (should still return current state)
    let future_result = runtime.view(
        "blocktracker".to_string(),
        &vec![],
        10, // Future height
    ).await.expect("Failed to query at future height");

    println!("Future (height 10): {} bytes: {:?}", future_result.len(), future_result);
    assert_eq!(future_result.len(), 5, "Future height should return current state (5 bytes)");
}