...
impl RockshrewDiffRuntime {
    ...
    fn print_diff_report(
        ...
        let report =
            self.generate_diff_report(height, primary_only_keys, compare_only_keys, value_diffs);
        print!("{}", report);
    }
    ...
    pub async fn process_block(&mut self, block_data: Vec<u8>, height: u32) -> Result<bool> {
        ...
        // Compare the updates
        let (primary_only_keys, compare_only_keys, value_diffs) =
            self.compare_updates(&primary_updates, &compare_updates);
        ...
        if !primary_only_keys.is_empty() || !compare_only_keys.is_empty() || !value_diffs.is_empty()
        {
            ...
            if let Err(e) =
                self.save_diff_report(height, &primary_only_keys, &compare_only_keys, &value_diffs)
            {
...
async fn main() -> Result<()> {
    ...
    let compare_runtime = MetashrewRuntime::load(compare_indexer, compare_tracking_adapter)?;
