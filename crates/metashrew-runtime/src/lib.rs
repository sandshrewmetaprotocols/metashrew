...
pub use crate::runtime::{MetashrewRuntime, MetashrewRuntimeContext};
...
pub async fn query_height(db: Arc<DB>, start_block: u32) -> Result<u32> {
    let height_key = runtime::TIP_HEIGHT_KEY.as_bytes().to_vec();
...
impl runtime::BatchLike for RocksDBBatch {
    fn default() -> Self {
...
impl runtime::KeyValueStoreLike for RocksDBRuntimeAdapter {
    ...
    fn write(&mut self, batch: RocksDBBatch) -> Result<(), Self::Error> {
        let key_bytes: Vec<u8> = runtime::TIP_HEIGHT_KEY.as_bytes().to_vec();
