...
pub async fn fetch_and_set_height(internal_db: &DynamoDbRuntimeAdapter) -> Result<u32> {
    let height = query_height(internal_db)
        .await
...
async fn main() -> std::io::Result<()> {
    ...
    let program = File::open(path.clone()).expect("Failed to open program file");
    ...
    if let Ok(label) = env::var("DYNAMODB_TABLE_PREFIX") {
