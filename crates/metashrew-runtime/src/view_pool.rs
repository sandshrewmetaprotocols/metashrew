...
impl<T: KeyValueStoreLike + Clone + Send + Sync + 'static> ViewRuntimePool<T> {
    ...
    pub async fn new(
        ...
        for i in 0..config.pool_size {
            ...
            if config.enable_logging {
                log::debug!("Created stateful view runtime {} of {}", i + 1, config.pool_size);
            }
        }
    ...
    pub async fn view(&self, symbol: String, input: &Vec<u8>, height: u32) -> Result<Vec<u8>> {
        ...
        if self.config.enable_logging {
            log::debug!(
                ...
                symbol, height, runtime_index
            );
        }
        
        // Execute the view function using the selected runtime
        let result = self.execute_view_with_runtime(
            runtime,
            symbol.clone(),
            input,
            height
    ...
    async fn execute_view_with_runtime(
        ...
        {
            let mut context_guard = runtime_guard.context.lock()
        ...
        let result_data = crate::runtime::read_arraybuffer_as_vec(
    ...
    pub async fn get_stats(&self) -> ViewPoolStats {
        ...
        ViewPoolStats {
            ...
            max_concurrent_requests: self.semaphore.available_permits() + 
                (self.config.max_concurrent_requests.unwrap_or(self.config.pool_size * 2) - available_permits),
        ...
        if new_size > current_size {
            ...
            for i in current_size..new_size {
                let runtime = StatefulViewRuntime::new(
                    ...
                    .with_context(|| format!("Failed to create additional stateful view runtime {}", i))?;
...
pub trait ViewPoolSupport<T: KeyValueStoreLike> {
    ...
    fn create_view_pool(&self, config: ViewPoolConfig) -> impl std::future::Future<Output = Result<ViewRuntimePool<T>>> + Send;
}

impl<T: KeyValueStoreLike + Clone + Send + Sync + 'static> ViewPoolSupport<T> for MetashrewRuntime<T> {
    /// Create a view runtime pool from this runtime
    ///
