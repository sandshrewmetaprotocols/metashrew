...
async fn test_metashrew_height_no_hanging() -> Result<()> {
    ...
    assert!(
        ...
        "Height queries should be very fast"
    );
    println!(
        "✓ Height queries are non-blocking and fast ({:?})",
        duration
    );
...
async fn test_height_consistency() -> Result<()> {
    ...
    for handle in handles {
        ...
        assert_eq!(
            height, 9,
            "All concurrent reads should return the same height"
        );
        println!(
            "✓ Concurrent read {} returned correct height: {}",
            i, height
        );
    }
