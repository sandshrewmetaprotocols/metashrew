...
/opt/metashrew/target/release/metashrew --log-filters $_LOG_FILTERS --db-dir $_DB_DIR --daemon-dir $_DAEMON_DIR --network $_NETWORK --daemon-rpc-addr $_DAEMON_RPC_ADDR --daemon-p2p-addr $_DAEMON_P2P_ADDR --electrum-rpc-addr ${_ELECTRUM_RPC_ADDR} --indexer $_INDEXER
