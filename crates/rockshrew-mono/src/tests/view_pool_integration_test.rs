...
async fn create_test_adapter() -> Result<(MetashrewRuntimeAdapter, tempfile::TempDir, tempfile::TempDir)> {
...
fn create_test_wasm_module() -> Result<(std::path::PathBuf, tempfile::TempDir)> {
    ...
    let minimal_wasm = vec![
        ...
        0x01, 0x04, 0x01, 0x60, 0x00, 0x00, // type section: 1 type, function type with no params/results
...
async fn test_view_execution_without_pool() -> Result<()> {
    ...
    assert!(!adapter.is_stateful_views_enabled().await, "Stateful views should be disabled");
...
async fn test_concurrent_view_execution() -> Result<()> {
    ...
    let stats = adapter.get_view_pool_stats().await.unwrap();
    assert!(stats.total_requests_processed >= 5, "All requests should be tracked");
