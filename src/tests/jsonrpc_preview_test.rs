...
use rockshrew_sync::{
    MetashrewSync, SyncConfig, JsonRpcProvider, RuntimeAdapter,
...
async fn test_metashrew_preview_jsonrpc() -> Result<()> {
    ...
    // Manually process blocks (since we're testing the JSON-RPC interface, not the sync engine)
    {
        ...
        for (height, block) in chain.iter().enumerate() {
            ...
            runtime_guard.process_block(height as u32, &block_bytes).await?;
        }
    }
    ...
    let result = sync_engine.metashrew_preview(
        block_hex.clone(),
        function_name.clone(),
        input_hex.clone(),
        height.clone(),
    ...
    assert_eq!(result_bytes.len(), expected_length,
              "Preview should show blocktracker with {} bytes", expected_length);
    
    println!("  ✓ Preview returned {} bytes (expected {})", result_bytes.len(), expected_length);
    ...
    let getblock_result = sync_engine.metashrew_preview(
        block_hex.clone(),
        "getblock".to_string(),
        height_input_hex,
        height.clone(),
    ...
    assert!(!getblock_bytes.is_empty(), "Getblock preview should return data");
    
    println!("  ✓ Getblock preview returned {} bytes", getblock_bytes.len());
    
    // Test 3: Preview with "latest" height
    println!("🧪 Test 3: Preview with 'latest' height");
    
    let latest_result = sync_engine.metashrew_preview(
        block_hex.clone(),
        function_name.clone(),
        input_hex.clone(),
        "latest".to_string(),
    ...
    assert!(!latest_bytes.is_empty(), "Preview with 'latest' should return data");
    
    println!("  ✓ Preview with 'latest' height returned {} bytes", latest_bytes.len());
    
    // Test 4: Preview should not affect main state
    println!("🧪 Test 4: Preview isolation test");
    
    // Get current state before preview
    let before_preview = sync_engine.metashrew_view(
        function_name.clone(),
        input_hex.clone(),
        height.clone(),
    ).await?;
    ...
    // Run preview
    let _preview_result = sync_engine.metashrew_preview(
        block_hex.clone(),
        function_name.clone(),
        input_hex.clone(),
        height.clone(),
    ...
    let after_preview = sync_engine.metashrew_view(
        function_name.clone(),
        input_hex.clone(),
        height.clone(),
    ).await?;
    ...
    assert_eq!(before_bytes, after_bytes, "Preview should not affect main state");
    ...
    println!("  ✓ Preview did not affect main state (both have {} bytes)", before_bytes.len());
    ...
    let error_result = sync_engine.metashrew_preview(
    ...
    assert!(error_result.is_err(), "Invalid block data should cause error");
    println!("  ✓ Invalid block data properly rejected");
    
    // Test with invalid view function
    let invalid_function_result = sync_engine.metashrew_preview(
        block_hex.clone(),
    ...
    assert!(invalid_function_result.is_err(), "Invalid view function should cause error");
    println!("  ✓ Invalid view function properly rejected");
    
    // Test with invalid height
    let invalid_height_result = sync_engine.metashrew_preview(
        block_hex.clone(),
        function_name.clone(),
        input_hex.clone(),
    ...
    assert!(invalid_height_result.is_err(), "Invalid height should cause error");
...
async fn test_metashrew_preview_complex_scenarios() -> Result<()> {
    ...
    for test_height in [0, 5, 9] {
        ...
        assert_eq!(result_bytes.len(), expected_length,
                  "Preview at height {} should show {} bytes", test_height, expected_length);
        
        println!("  ✓ Preview at height {} returned {} bytes", test_height, result_bytes.len());
    }
    ...
    for i in 0..3 {
        ...
        assert!(!result_bytes.is_empty(),
                "Consecutive preview {} should return data", i);
        
        println!("  ✓ Consecutive preview {} returned {} bytes", i, result_bytes.len());
    }
    ...
    let large_result = sync_engine.metashrew_preview(
        block_hex.clone(),
        "blocktracker".to_string(),
        large_input_hex,
        "latest".to_string(),
    ...
    assert!(!large_result_bytes.is_empty(), "Preview with large input should work");
    
    println!("  ✓ Preview with large input (1KB) returned {} bytes", large_result_bytes.len());
...
async fn test_metashrew_preview_performance() -> Result<()> {
    ...
    println!("📦 Processing {} blocks for performance testing", chain.len());
    ...
    for i in 0..num_calls {
        let _result = sync_engine.metashrew_preview(
            block_hex.clone(),
            "blocktracker".to_string(),
            input_hex.clone(),
            "latest".to_string(),
    ...
    assert!(avg_time.as_millis() < 1000, "Average preview time should be under 1 second");
    assert!(calls_per_second > 1.0, "Should handle at least 1 call per second");
