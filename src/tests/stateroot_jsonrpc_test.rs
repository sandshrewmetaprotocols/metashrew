...
async fn test_stateroot_calculation_and_retrieval() -> Result<()> {
            ...
            Ok(state_root) => {
                ...
                if state_root == zero_root {
                    return Err(anyhow::anyhow!(
                        "State root for height {} is all zeros!",
                        height
                    ));
                }
            }
    ...
    for height in 0..chain.len() as u32 {
        ...
        match smt_helper.get_smt_root_at_height(height) {
            Ok(root) => {
                println!(
                    "✅ JSON-RPC compatible retrieval for height {}: 0x{}",
                    height,
                    hex::encode(&root)
                );
                ...
                if root.is_empty() {
                    return Err(anyhow::anyhow!(
                        "State root for height {} is empty!",
                        height
                    ));
                }
            }
            Err(e) => {
                return Err(anyhow::anyhow!(
                    "JSON-RPC compatible retrieval failed for height {}: {}",
                    height,
                    e
                ));
            }
    ...
    match smt_helper.get_smt_root_at_height(latest_height) {
        Ok(root) => {
            println!(
                "✅ State root for 'latest' (height {}): 0x{}",
                latest_height,
                hex::encode(&root)
            );
        }
        Err(e) => {
            return Err(anyhow::anyhow!(
                "Failed to get state root for 'latest' height {}: {}",
                latest_height,
                e
            ));
        }
...
async fn test_stateroot_key_format_consistency() -> Result<()> {
    ...
    match adapter.get(wasm_key.as_bytes()) {
        Ok(Some(value)) => {
            println!(
                "✅ Found state root with WASM key format: 0x{}",
                hex::encode(&value)
            );
        }
        Ok(None) => {
    ...
    match adapter.get(legacy_key.as_bytes()) {
        Ok(Some(value)) => {
            println!(
                "⚠️  Found state root with legacy key format: 0x{}",
                hex::encode(&value)
            );
        }
        Ok(None) => {
    ...
    match smt_helper.get_smt_root_at_height(height) {
        Ok(root) => {
            println!(
                "✅ SMTHelper retrieved state root: 0x{}",
                hex::encode(&root)
            );
        }
        Err(e) => {
            return Err(anyhow::anyhow!(
                "SMTHelper failed to retrieve state root: {}",
                e
            ));
        }
    }
...
async fn test_empty_vs_missing_stateroot() -> Result<()> {
    ...
    match smt_helper.get_smt_root_at_height(0) {
        Ok(root) => {
            println!(
                "⚠️  Unexpectedly found state root for unprocessed height 0: 0x{}",
                hex::encode(&root)
            );
            ...
            if root == zero_root {
                ...
                return Err(anyhow::anyhow!(
                    "State root should not exist for unprocessed blocks"
                ));
            }
        }
        Err(_) => {
    ...
    match smt_helper.get_smt_root_at_height(999999) {
        Ok(root) => {
            println!(
                "❌ Unexpectedly found state root for height 999999: 0x{}",
                hex::encode(&root)
            );
            return Err(anyhow::anyhow!(
                "State root should not exist for unprocessed blocks"
