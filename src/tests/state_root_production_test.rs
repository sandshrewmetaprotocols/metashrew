...
fn test_state_root_with_rocksdb_adapter() -> Result<()> {
    ...
    for (key, value) in &test_data {
        match smt_helper.bst_put(key, value, height) {
            ...
            Err(e) => {
                println!("DEBUG: Failed to store key: {:?}, error: {:?}", String::from_utf8_lossy(key), e);
                return Err(e);
            }
        }
    }
    ...
    match smt_helper.get_smt_root_at_height(1) {
        Ok(retrieved_root) => {
            println!("DEBUG: Retrieved state root for height 1: {}", hex::encode(retrieved_root));
...
fn test_state_root_sequential_heights() -> Result<()> {
    ...
    for height in 0..3 {
        ...
        match smt_helper.calculate_and_store_state_root(height) {
            Ok(state_root) => {
                println!("DEBUG: State root for height {}: {}", height, hex::encode(state_root));
            },
            Err(e) => {
                println!("ERROR: Failed to calculate state root for height {}: {:?}", height, e);
                return Err(e);
            }
        }
    }
    ...
    for height in 0..3 {
        match smt_helper.get_smt_root_at_height(height) {
            Ok(state_root) => {
                println!("DEBUG: Retrieved state root for height {}: {}", height, hex::encode(state_root));
            },
            Err(e) => {
                println!("ERROR: Failed to retrieve state root for height {}: {:?}", height, e);
                return Err(e);
            }
        }
    }
