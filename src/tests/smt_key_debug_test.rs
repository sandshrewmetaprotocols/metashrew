...
async fn test_smt_key_storage_and_retrieval() -> Result<()> {
    ...
    for (key, value) in &all_data {
        ...
        if key_str.contains("smt:root") {
            println!("Found SMT root key: '{}' -> {} bytes: 0x{}", key_str, value.len(), hex::encode(value));
        }
    }
    ...
    match smt_helper.get_smt_root_at_height(height) {
        Ok(root) => {
            println!("SMTHelper.get_smt_root_at_height({}) returned: 0x{}", height, hex::encode(&root));
        ...
        Err(e) => {
            println!("❌ SMTHelper.get_smt_root_at_height({}) failed: {}", height, e);
        }
    }
    ...
    for key_format in formats {
        match adapter.get(key_format.as_bytes()) {
            Ok(Some(value)) => {
                println!("✅ Found value with key '{}': {} bytes: 0x{}", key_format, value.len(), hex::encode(&value));
            },
            Ok(None) => {
...
async fn test_smt_helper_storage_mechanism() -> Result<()> {
    ...
    let test_root = [0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88,
    ...
    println!("Manually storing test root at height {} with key '{}'", test_height, String::from_utf8_lossy(&root_key));
