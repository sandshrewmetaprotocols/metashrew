...
async fn test_surface_api_complete_workflow() -> Result<()> {
    ...
    for height in 0..chain.len() {
        ...
        let view_input = vec![];
        let blocktracker_result = runtime
            .view("blocktracker".to_string(), &view_input, height_u32)
            .await?;
        ...
        let block_result = runtime
            .view("getblock".to_string(), &height_input, height_u32)
            .await?;
        ...
        println!(
            "  ✓ View functions at height {}: blocktracker={} bytes, block={} bytes",
            height,
            blocktracker_result.len(),
            block_result.len()
        );
    ...
    assert_eq!(
        preview_result.len(),
        expected_preview_length,
        "Preview blocktracker should have {} bytes",
        expected_preview_length
    );

    println!(
        "  ✓ Preview function: blocktracker would have {} bytes after new block",
        preview_result.len()
    );
    ...
    for height in 0..chain.len() {
        ...
        assert_eq!(
            result1, result2,
            "Historical queries should be deterministic at height {}",
            height
        );
        assert_eq!(
            result2, result3,
    ...
    let future_result = runtime
        .view("blocktracker".to_string(), &view_input, future_height)
        .await?;

    // Should return empty or last known state
    println!(
        "  ✓ Future height query handled: {} bytes",
        future_result.len()
    );
    ...
    assert_eq!(
        genesis_result.len(),
        1,
        "Genesis blocktracker should have 1 byte"
    );
    ...
    let invalid_result = runtime
        .view("getblock".to_string(), &invalid_height_input, 0)
        .await;

    // Should handle gracefully (either error or empty result)
    match invalid_result {
        Ok(result) => println!(
            "  ✓ Invalid height input handled gracefully: {} bytes",
            result.len()
        ),
...
async fn test_surface_api_performance() -> Result<()> {
    ...
    println!(
        "📦 Block processing: {} blocks in {:?} ({:.2} blocks/sec)",
        chain.len(),
        processing_time,
    ...
    for height in 0..chain.len() {
        let view_input = vec![];
        let _result = runtime
            .view("blocktracker".to_string(), &view_input, height as u32)
            .await?;
        total_queries += 1;

        let height_input = (height as u32).to_le_bytes().to_vec();
        let _result = runtime
            .view("getblock".to_string(), &height_input, height as u32)
            .await?;
        total_queries += 1;
    }
    ...
    println!(
        "🔍 View queries: {} queries in {:?} ({:.2} queries/sec)",
        total_queries,
        query_time,
    ...
    assert!(
        ...
        "Block processing should complete within 5 seconds"
    );
    assert!(
        query_time.as_millis() < 2000,
        "View queries should complete within 2 seconds"
    );
    assert!(
        preview_time.as_millis() < 1000,
        "Preview should complete within 1 second"
    );
...
async fn test_surface_api_error_handling() -> Result<()> {
    ...
    let view_input = vec![];
    let invalid_view_result = runtime
        .view("nonexistent_function".to_string(), &view_input, 0)
        .await;
    ...
    let valid_result = runtime
        .view("blocktracker".to_string(), &view_input, 0)
        .await?;
    assert_eq!(
        valid_result.len(),
        1,
        "Valid view function should work after error"
    );
    ...
    assert_eq!(
        valid_preview_result.len(),
        5,
        "Valid preview should work after error"
    );
    ...
    for height in 0..chain.len() {
        ...
        assert_eq!(
            result.len(),
            height + 1,
            "Concurrent view should work at height {}",
            height
        );
    }
...
async fn test_surface_api_complex_inputs() -> Result<()> {
    ...
    let empty_input = vec![];
    let result = runtime
        .view("blocktracker".to_string(), &empty_input, 2)
        .await?;
    ...
    for height in 0..chain.len() {
        ...
        let result = runtime
            .view("getblock".to_string(), &correct_input, height_u32)
            .await?;
        assert!(
            !result.is_empty(),
            "Correct input should return data at height {}",
            height
        );
        ...
        let result = runtime
            .view("getblock".to_string(), &oversized_input, height_u32)
            .await?;
        assert!(
            !result.is_empty(),
            "Oversized input should work at height {}",
            height
        );
    ...
    let result = runtime
        .view("blocktracker".to_string(), &large_input, 2)
        .await?;
    ...
    let result = runtime
        .view("blocktracker".to_string(), &binary_input, 1)
        .await?;
...
async fn test_surface_api_state_isolation() -> Result<()> {
    ...
    let view_input = vec![];
    let result_h0 = runtime
        .view("blocktracker".to_string(), &view_input, 0)
        .await?;
    let result_h1 = runtime
        .view("blocktracker".to_string(), &view_input, 1)
        .await?;
    let result_h2 = runtime
        .view("blocktracker".to_string(), &view_input, 2)
        .await?;

    assert_ne!(
        result_h0, result_h1,
        "Different heights should return different results"
    );
    assert_ne!(
        result_h1, result_h2,
        "Different heights should return different results"
