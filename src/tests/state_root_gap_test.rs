...
fn test_state_root_with_gaps() -> Result<()> {
    ...
    for height in 0..3 {
        ...
        println!("DEBUG: Stored state root for height {}: {}", height, hex::encode(state_root));
    }
    ...
    println!("DEBUG: Simulating gap - trying to process height {} without previous state roots", gap_height);
    ...
    match smt_helper.calculate_and_store_state_root(gap_height) {
        Ok(state_root) => {
            println!("DEBUG: Unexpectedly succeeded in calculating state root for height {}: {}", gap_height, hex::encode(state_root));
        },
        Err(e) => {
            println!("ERROR: Failed to calculate state root for height {}: {:?}", gap_height, e);
...
fn test_state_root_recovery_strategy() -> Result<()> {
    ...
    for (key, _) in smt_helper.storage.scan_prefix(prefix)?.into_iter() {
        ...
        if let Some(rest) = key_str.strip_prefix(metashrew_runtime::smt::BST_HEIGHT_PREFIX) {
            ...
            if let Some(colon_pos) = rest.find(':') {
                ...
                if let (Ok(original_key), Ok(key_height)) = (hex::decode(hex_key), height_str.parse::<u32>()) {
    ...
    println!("DEBUG: Found {} unique keys in database up to height {}", all_keys.len(), height);
    
    // Build current state map
    let mut current_state: std::collections::BTreeMap<Vec<u8>, Vec<u8>> = std::collections::BTreeMap::new();
    for key in all_keys {
    ...
    println!("DEBUG: Built current state with {} key-value pairs", current_state.len());
    ...
    println!("DEBUG: Recovery state root calculated and stored: {}", hex::encode(recovery_root));
    
    // Verify we can retrieve it
    match smt_helper.get_smt_root_at_height(height) {
        Ok(retrieved_root) => {
            println!("DEBUG: Successfully retrieved recovery state root: {}", hex::encode(retrieved_root));
            assert_eq!(retrieved_root, recovery_root, "Retrieved root should match calculated root");
        },
        Err(e) => {
